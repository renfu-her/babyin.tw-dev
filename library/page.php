...
class SplitPage {
   ...
   function setViewList($url, $target = false) {  //產生導覽列
     ...
     if($this->total_pages > 1) {
       if(($this->page - 1) != 0) {
         $this->viewlist .= "每頁顯示-".$this->records_per_page."筆　<a href='{$url}page=" . ($this->page - 1) ."'{$target}>上一頁</a>　";
       ...
       if(($this->page + 1) <= $this->total_pages) {
         ...
         $this->viewlist .= "<a href='{$url}page=" . ($this->page + 1) . "'{$target}>下一頁</a>　共 ".$this->total_pages." 頁";
       }else{
