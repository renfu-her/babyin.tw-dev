<?php
			...
			if($ary_post['proc'] == 'add'){
				$total_data = $obj_faq_category->fetch('count(*) total' , " pkey != '0' ");
				$ary_DBdata['sort'] = ($total_data['total'] * 5 +200);

				if($obj_faq_category->create($ary_DBdata)){
				...
				$ary_data = $obj_faq_category->fetch('*' , " pkey = '".$ary_post['pk']."' ");
				if($obj_faq_category->edit($ary_DBdata , " pkey = '".$ary_post['pk']."' ")){
