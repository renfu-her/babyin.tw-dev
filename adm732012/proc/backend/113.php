<?php
			...
			if($ary_post['proc'] == 'add'){
				...
				$total_data = $obj_faq->fetch('count(*) total' , " data_pkey = '".$ary_post['data_pkey']."' ");
				$ary_DBdata['sort'] = ($total_data['total'] * 5 +200);

				if($obj_faq->create($ary_DBdata)){
				...
				$ary_data = $obj_faq->fetch('*' , " pkey = '".$ary_post['pk']."' ");
				if($obj_faq->edit($ary_DBdata , " pkey = '".$ary_post['pk']."' ")){
